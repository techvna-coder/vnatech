...
# Hằng số header để tách theo slide/page
TEXT_SLIDE_HEADER = "--- Slide {idx} ---"

def _normalize_whitespace(s: str) -> str:
    """Rút gọn khoảng trắng (space/tab lặp), trim hai đầu; an toàn với None."""
    if s is None:
        return ""
        ...
        for shape in slide.shapes:
            ...
            if getattr(shape, "has_text_frame", False) and getattr(shape, "text_frame", None):
                try:
                    ...
                    normed = [_normalize_whitespace(t) for t in paras]
                    txt = "\n".join([t for t in normed if t])
                except Exception:
                    # Fallback an toàn
            ...
            if getattr(shape, "has_table", False):
                try:
                    ...
                    errors.append(f"slide {s_idx} table: {e}")  # bỏ qua lỗi cục bộ
...
def chunk_text_smart(text: str, chunk_size: int = 1200, chunk_overlap: int = 150) -> List[str]:
    ...
    if len(parts) > 1:
        ...
        while i < len(parts):
            seg = parts[i]
            if re.match(r"---\s*(Slide|Page)\s+\d+\s*---", seg):
                header = seg
